<div class="border border-secondary p-4 rounded bg-white col-sm-8 col-lg-12 mx-auto">
	<form method="POST" action="/login"  onsubmit="return (checkFieldEmail('emailid') &&checkFieldPassword('password'))">
		...
		<input type="email" class="form-control mt-4" name="emailid" id="emailid"  placeholder="Enter Email Address *" value="<?=$emailid?>" onkeyup="checkFieldEmail('emailid')">
		...
		<div class="row mt-4 m-2">
			...
			<p class="text-muted text-center d-inline col-5 mt-1">or connect with</p>
			<hr class="d-inline col">
		</div>         
		<a href="<?=$loginURL?>" >
			<img src="../resources/images/google_logo.jpg" class="rounded-circle mx-auto d-block m-2 p-2"  alt="Login with Google" height="70"  onMouseOver="this.style.boxShadow = '0px 0px 3px #000'; this.style.transitionDuration = '0.3s';" onMouseOut="this.style.boxShadow = '0 0 0 #000'; this.style.transitionDuration = '0.3s';">
		</a>
	</div>
</form>          
</div>
